---
type: entity
title: Cat Wu
aliases:
  - Cat Wu
  - Catherine Wu
definition: "Anthropic Claude Code 和 Cowork 产品负责人。普林斯顿 CS 专业，Scale AI 早期工程师，Index Ventures 风投经历。2024 年加入 Anthropic，与 Boris Cherny 搭档将 Claude Code 推向 300 亿美元级增长引擎"
validated_source: "https://www.linkedin.com/in/cat-wu"
validated_at: "2026-05-08"
created: 2026-05-08
updated: 2026-05-08
tags:
  - AI
  - product-management
  - claude-code
source_raw:
  - "[[How Anthropic's product team moves faster than anyone else | Cat Wu (Head of Product, Claude Code)]]"
---

# Cat Wu

> [!definition] 定义
> **Cat Wu**（Catherine Wu）是 Anthropic Claude Code 和 Cowork 的产品负责人。普林斯顿大学计算机科学专业，曾任 Scale AI 产品工程师、Dagster 工程经理、Index Ventures 风投投资人。2024 年 8 月加入 Anthropic，与 Boris Cherny 搭档。2025 年 7 月曾被 Cursor 挖角，两周后随 Boris 一同回归，全面接管 Claude Code 产品线。她面试过数百位试图进入 AI 领域的 PM，看到 firsthand 哪些人在成功、哪些人已落后。

## 职业履历

| 时间 | 事件 |
|------|------|
| 大学 | 普林斯顿大学计算机科学专业 |
| 早期 | Scale AI 产品工程师 |
| 2020-2024 | Index Ventures 风投投资人（服务 Figma、Datadog、Discord 等） |
| 2024 年 8 月 | 加入 Anthropic |
| 2025 年 7 月 | 被 Cursor（Anysphere）挖角，两周后回归 Anthropic |
| 2025-2026 | 与 Boris Cherny 搭档领导 Claude Code 产品，推动发布周期从 6 个月降至 1 天 |

## 核心思想贡献

- **AI-Native Shipping**: 产品发布周期从月到天——不是靠更好模型，而是靠流程和文化
- **Research Preview 策略**: 降低发布门槛的核心方法论
- **PM 角色转型**: PM 从 roadmap 对齐者变为"高速公路设计师"
- **Model Introspection**: 最 underrated 的 AI 技能——让模型自省错误
- **Mission Alignment 为组织效率**: Anthropic 极速迭代的底层驱动
- **Delete Features > Add Features**: 新模型让删除功能比新增功能更有价值
